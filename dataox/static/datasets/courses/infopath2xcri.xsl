...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://xcri.org/profiles/1.2/catalog" xmlns:xcriTerms="http://xcri.org/profiles/catalog/terms" xmlns:xcri="http://xcri.org/profiles/1.2/catalog" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:oxcap="http://purl.ox.ac.uk/oxcap/ns/" xmlns:credit="http://purl.org/net/cm" xmlns:mlo="http://purl.org/net/mlo" xmlns:courseDataProgramme="http://xcri.co.uk" xmlns:oxnotation="https://data.ox.ac.uk/id/notation/" xmlns:ox-rm="https://data.ox.ac.uk/id/ox-rm/" xmlns:ox-rdf="https://data.ox.ac.uk/id/ox-rdf/" xmlns:sharepoint="https://github.com/ox-it/python-sharepoint/" xpath-default-namespace="http://schemas.microsoft.com/office/infopath/2003/myXSD/2012-03-17T23:37:18" version="2.0">
  ...
  <xsl:template match="myFields">
    <provider>
      <xsl:choose>
        <xsl:when test="txt_description">
          <dc:description>
            <xsl:apply-templates select="txt_description"/>
          </dc:description>
        </xsl:when>
        <xsl:otherwise>
          <dc:description>University of Oxford</dc:description>
        </xsl:otherwise>
      </xsl:choose>
      ...
      <!-- ptitle -->
      <dc:title>
        <xsl:value-of select="txt_ptitle"/>
      </dc:title>
      <mlo:url>
        <xsl:value-of select="txt_url"/>
      </mlo:url>
      <!-- course, with oxcap:visibility and possibly oxcap:status attributes -->
      <course>
        <xsl:choose>
          <xsl:when test="normalize-space(cmb_visibility)='PB' or not(cmb_visibility)">
            <xsl:attribute name="oxcap:visibility">PB</xsl:attribute>
          </xsl:when>
          <xsl:when test="normalize-space(cmb_visibility)='RS'">
            <xsl:attribute name="oxcap:visibility">RS</xsl:attribute>
          </xsl:when>
          <xsl:when test="normalize-space(cmb_visibility)='PR'">
            <xsl:attribute name="oxcap:visibility">PR</xsl:attribute>
          </xsl:when>
          <xsl:otherwise>
            <xsl:message>
                  Course gives unknown visibility of <xsl:value-of select="cmb_visibility"/></xsl:message>
          </xsl:otherwise>
        </xsl:choose>
        <xsl:if test="string-length(normalize-space(cmb_status)) gt 0 and not(normalize-space(cmb_status) = 'CN')">
          <xsl:attribute name="oxcap:status">
            <xsl:value-of select="normalize-space(cmb_status)"/>
          </xsl:attribute>
        </xsl:if>
        <dc:identifier>
        ...
        <dc:title>
          <xsl:value-of select="txt_ctitle"/>
        </dc:title>
        <mlo:url>
          <xsl:value-of select="txt_curl"/>
        </mlo:url>
        <!-- all are graduate training -->
        <dc:subject xsi:type="ox-rdf:notation" identifier="GT">Graduate Training</dc:subject>
        <!-- subject -->
        <xsl:if test="string-length(normalize-space(txt_subject)) gt 0">
          <dc:subject>
            <xsl:value-of select="txt_subject"/>
          </dc:subject>
        </xsl:if>
        <!-- skills -->
        ...
        <regulations oxcap:eligibility="{normalize-space(cmb_prerequisite)}">
          <div xmlns="http://www.w3.org/1999/xhtml">
            <xsl:choose>
              <xsl:when test="upper-case(normalize-space(cmb_prerequisite)) = 'OX' or not(cmb_prerequisite)">
                <p>This course is available to the current students and staff of the University of Oxford.</p>
              </xsl:when>
              <xsl:when test="upper-case(normalize-space(cmb_prerequisite)) = 'ST'">
                <p>This course is available to the current staff of the University of Oxford.</p>
              </xsl:when>
              <xsl:when test="upper-case(normalize-space(cmb_prerequisite)) = 'PU'">
        ...
        <xsl:if test="string-length(normalize-space(txt_abstract)) gt 0">
          <learningOutcomes>
            <xsl:apply-templates select="txt_abstract"/>
          </learningOutcomes>
        </xsl:if>
        ...
        <presentation>
          <xsl:if test="string-length(normalize-space(cmb_status)) gt 0 and normalize-space(cmb_status) = 'CN'">
            <xsl:attribute name="oxcap:status">
              <xsl:value-of select="normalize-space(cmb_status)"/>
            </xsl:attribute>
          </xsl:if>
          ...
          <!-- Start /end -->
          <mlo:start>
            <xsl:if test="dt_start">
              <xsl:attribute name="dtf">
                <xsl:value-of select="normalize-space(dt_start)"/>
              </xsl:attribute>
            </xsl:if>
            <xsl:choose>
              <xsl:when test="string-length(normalize-space(txt_applyfromtext)) gt 0">
                <xsl:value-of select="txt_starttext"/>
              </xsl:when>
          ...
          <end>
            <xsl:if test="dt_end">
              <xsl:attribute name="dtf">
                <xsl:value-of select="normalize-space(dt_end)"/>
              </xsl:attribute>
            </xsl:if>
            <xsl:choose>
              <xsl:when test="string-length(normalize-space(dt_start)) gt 0">
                <xsl:value-of select="format-date(xs:date(dt_end), '[F] [D] [MNn] [Y]')"/>
              </xsl:when>
              <xsl:otherwise>
          ...
          <oxcap:memberApplyTo>
            <xsl:value-of select="txt_applyTo"/>
          </oxcap:memberApplyTo>
          <applyTo>
            <xsl:value-of select="txt_publicapplyto"/>
          </applyTo>
          <!-- Venue -->
          ...
          <applyFrom>
            <xsl:if test="dt_applyfrom">
              <xsl:attribute name="dtf">
                <xsl:value-of select="normalize-space(dt_applyfrom)"/>
              </xsl:attribute>
            </xsl:if>
            <xsl:choose>
              <xsl:when test="string-length(normalize-space(txt_applyfromtext)) gt 0">
              ...
              <xsl:when test="dt_applyfrom[not(@xsi:nil)]">
                <xsl:value-of select="format-date(xs:date(dt_applyfrom), '[F] [D] [MNn] [Y]')"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:message>Error: Missing start date!</xsl:message>
              </xsl:otherwise>
          ...
          <applyUntil>
            <xsl:if test="dt_applyuntil">
              <xsl:attribute name="dtf">
                <xsl:value-of select="normalize-space(dt_applyuntil)"/>
              </xsl:attribute>
            </xsl:if>
            <xsl:choose>
              ...
              <xsl:when test="dt_applyuntil[not(@xsi:nil)]">
                <xsl:value-of select="format-date(xs:date(dt_applyuntil), '[F] [D] [MNn] [Y]')"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:message>Error: Missing end date!</xsl:message>
              </xsl:otherwise>
          ...
          <attendanceMode identifier="{$attendanceModeIn}">
            ...
            <xsl:value-of select="$attendanceModeText"/>
          </attendanceMode>
          ...
          <attendancePattern identifier="{$attendancePatternIn}">
            ...
            <xsl:value-of select="$attendancePatternText"/>
          </attendancePattern>
